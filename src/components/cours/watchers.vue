<template>
    ...
    <section id="theory" class="mb-8">
      <div class="card bg-base-100 shadow-xl rounded-box p-6">
        ...
        <p class="text-base-content opacity-80">
          Un watcher est une fonction déclarative utilisée pour réagir aux
          changements d'une donnée réactive ou d'une propriété calculée. Ils
          sont souvent utilisés pour des tâches complexes ou lorsque vous avez
          besoin d'effectuer des actions spécifiques suite à un changement.
        </p>
      ...
      <div class="lg:w-1/2">
        <div class="card bg-base-100 shadow-xl rounded-box p-6">
          ...
          <p class="text-base-content opacity-80 mb-4">
            Cet exemple montre comment un watcher peut être utilisé pour
            détecter lorsque le compteur dépasse une certaine limite :
          </p>
  ...
  &lt;/script&gt;
          ...
          <div class="p-4 bg-base-200 rounded-box shadow-md">
            <p class="text-base-content opacity-80">
              Compteur :
              <span class="badge badge-lg badge-primary">{{ counter }}</span>
            </p>
            <button class="btn btn-primary mt-3" @click="increment">
              Incrémenter
            </button>
            <p class="mt-3 text-error font-bold" v-if="limitReached">
              ⚠️ Limite atteinte !
            </p>
          </div>
        </div>
      ...
      <div class="lg:w-1/2">
        <div class="card bg-base-100 shadow-xl rounded-box p-6">
          <h2 class="card-title text-2xl font-bold mb-3">
            Exemple 2 : Validation d'une chaîne de caractères
          </h2>
          <p class="text-base-content opacity-80 mb-4">
            Cet exemple montre comment un watcher peut être utilisé pour valider
  ...
  &lt;/script&gt;
          ...
          <div class="p-4 bg-base-200 rounded-box shadow-md">
            ...
            <p class="mt-3 text-error font-bold" v-if="errorMessage">
              {{ errorMessage }}
            </p>
          </div>
        </div>
